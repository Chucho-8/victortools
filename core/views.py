...
from .models import senales
from .serializers import senalesSerializer
...
class SenalesListAPIView(APIView):
    permission_classes = [AllowAny]

    def get(self, request):
        señales = senales.objects.all()
        serializer = senalesSerializer(señales, many=True)
        return Response(serializer.data)

    def post(self, request):
        serializer = senalesSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

class SenalDetailAPIView(APIView):
    permission_classes = [AllowAny]

    def get(self, request, pk):
        señal = get_object_or_404(senales, pk=pk)
        serializer = senalesSerializer(señal)
        return Response(serializer.data)

    def put(self, request, pk):
        señal = get_object_or_404(senales, pk=pk)
        serializer = senalesSerializer(señal, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def patch(self, request, pk):
        señal = get_object_or_404(senales, pk=pk)
        serializer = senalesSerializer(señal, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
